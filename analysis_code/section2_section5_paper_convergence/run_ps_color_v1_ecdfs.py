"""Apply the existing color-only Random Forest pS_color classifier to ECDFS.

This is an external-field sanity check.  The classifier is not retrained and
uses only the dust-corrected color features [ug, gr, ri, iz, zy].
"""
...
def _ensure_missing(paths: list[Path]) -> None:
    ...
    if existing:
        ...
        raise FileExistsError(f"Refusing to overwrite existing ECDFS pS_color_v1 outputs:\n{joined}")
...
def _truth_star_mask(df: pd.DataFrame) -> pd.Series:
    return df["truth_label"].astype("string").str.lower().eq("star")


def _truth_gal_mask(df: pd.DataFrame) -> pd.Series:
    return df["truth_label"].astype("string").str.lower().eq("galaxy")
...
def _summary_rows(ps_color: pd.DataFrame, classifier_path: Path) -> pd.DataFrame:
    rows: list[dict] = []
    truth_count = int((ps_color["truth_label"].astype("string").str.lower().isin(["star", "galaxy"])).sum())
    base = {
        "field": FIELD,
        ...
        "truth_label_source": "HST external labels",
    }
    for label, lo, hi in [("16 < r < 26", 16.0, 26.0), *[(f"{lo:g} < r < {hi:g}", lo, hi) for lo, hi in MAG_BINS]]:
        sub = ps_color[ps_color[RMAG_COL].gt(lo) & ps_color[RMAG_COL].lt(hi)]
        finite = sub[sub["valid_color_features"] & np.isfinite(sub[PS_COL])]
        ...
        rows.append(
            {
                **base,
                ...
                "N_truth_labeled": truth_sub if label != "16 < r < 26" else truth_count,
...
def _plot_truth_hist(ps_color: pd.DataFrame, output_png: Path) -> list[Path]:
    ...
    for ax, (lo, hi) in zip(axes.flat, MAG_BINS):
        ...
        if len(gal_vals):
            ax.hist(gal_vals, bins=hist_bins, density=True, histtype="step", lw=1.8, color=COLORS["galaxy"], label="HST galaxy")
        if len(star_vals):
            ax.hist(star_vals, bins=hist_bins, density=True, histtype="step", lw=1.8, color=COLORS["star"], label="HST star")
    ...
    fig.suptitle("ECDFS color-only RF star probability by HST label", y=0.965, fontsize=16)
...
def _plot_performance(summary: pd.DataFrame, output_png: Path) -> list[Path]:
    ...
    ax.set_title("ECDFS pS_color v1 star-positive performance")
...
def _write_report(
    ...
    lines = [
        ...
        f"- ECDFS DP2 analysis table: `{analysis_path.relative_to(repo_root)}`",
        f"- HST matched labels for evaluation only: `{matched_path.relative_to(repo_root)}`",
    ...
    if comparison_csv is not None:
        lines.append(f"- COSMOS vs ECDFS comparison CSV: `{comparison_csv.relative_to(repo_root)}`")
    lines += [
        "",
        "Performance summary:",
        f"- Total ECDFS paper sample: {int(overall['N_paper_sample']):,}",
        f"- Finite color-feature rows: {int(overall['N_finite_color_features']):,}",
        f"- HST truth-labeled rows: {int(overall['N_truth_labeled']):,}",
...
def _write_comparison(repo_root: Path, ecdfs_summary: pd.DataFrame) -> tuple[Path | None, Path | None]:
    cosmos_path = repo_root / "paper_convergence/results/section5_discussion/ps_color_v1_summary.csv"
    if not cosmos_path.exists():
        return None, None
    cosmos = pd.read_csv(cosmos_path)
    cosmos = cosmos.assign(field="COSMOS")
    ecdfs = ecdfs_summary.assign(field="ECDFS")
    cols = [
        "field",
        "bin_label",
        "N_paper_sample",
        "N_finite_color_features",
        "N_truth_labeled",
        "N_eval",
        "N_star",
        "N_galaxy",
    ...
    comparison = pd.concat([cosmos[cols], ecdfs[cols]], ignore_index=True)
    out_csv = repo_root / "paper_convergence/results/section5_discussion/ps_color_v1_cosmos_vs_ecdfs_summary.csv"
    out_md = repo_root / "paper_convergence/results/section5_discussion/ps_color_v1_cosmos_vs_ecdfs_summary.md"
    _ensure_missing([out_csv, out_md])
    comparison.to_csv(out_csv, index=False)
    _write_text(
        out_md,
        "\n".join(
            [
                "# pS_color v1 COSMOS vs ECDFS Summary",
                "",
                "This table compares the first-pass color-only classifier performance in COSMOS/COSMOS2020 and ECDFS/HST.",
                "",
                "Caveat: ECDFS/HST has far fewer truth-labeled stars and galaxies than COSMOS/COSMOS2020, so ECDFS per-bin metrics are noisier.",
                "",
                f"- CSV: `{out_csv.relative_to(repo_root)}`",
            ]
        )
        + "\n",
    )
...
def run(repo_root: Path) -> dict[str, list[Path]]:
    ...
    analysis_path = repo_root / "outputs/dp2_ecdfs_analysis_table.parquet"
    matched_path = repo_root / "outputs/dp2_ecdfs_hst_matched.parquet"
    output_parquet = repo_root / "outputs/dp2_ecdfs_ps_color_v1.parquet"
    ...
    summary_csv = results_dir / "ps_color_v1_ecdfs_summary.csv"
    summary_md = results_dir / "ps_color_v1_ecdfs_summary.md"
    report = docs_dir / "section5_ps_color_ecdfs_report.md"
    fig_hist_png = figures_dir / "fig5_2_ecdfs_ps_color_truth_hist_by_rmag.png"
    fig_perf_png = figures_dir / "fig5_3_ecdfs_ps_color_performance_by_rmag.png"
    fig_cc_png = figures_dir / "fig5_4_ecdfs_ps_color_method_color_color_2x4.png"
    derived_outputs = [
        report,
        fig_hist_png,
    ...
    _ensure_missing(derived_outputs)
    ...
    if output_parquet.exists():
        paper = pd.read_parquet(output_parquet, columns=out_cols)
    else:
        cols = ["object_id", "ra", "dec", RMAG_COL, *FEATURES]
        dp2 = pd.read_parquet(analysis_path, columns=cols)
        for col in [RMAG_COL, *FEATURES]:
            dp2[col] = pd.to_numeric(dp2[col], errors="coerce")
        paper = dp2.loc[dp2[RMAG_COL].gt(16.0) & dp2[RMAG_COL].lt(26.0)].copy()
        paper["valid_color_features"] = np.isfinite(paper[FEATURES]).all(axis=1)
        paper[PS_COL] = np.nan

        clf = joblib.load(classifier_path)
        valid = paper["valid_color_features"]
        paper.loc[valid, PS_COL] = clf.predict_proba(paper.loc[valid, FEATURES].to_numpy())[:, 1]
        paper["ps_color_version"] = VERSION
        paper["field"] = FIELD

        truth = pd.read_parquet(matched_path, columns=["object_id", "truth_label", "truth_binary", "external_label_source"])
    ...
    if summary_csv.exists() and summary_md.exists():
        ...
        _ensure_missing([summary_csv, summary_md])
        ...
        _write_text(
            ...
            "\n".join(
                [
                    ...
                    f"- Total ECDFS paper sample: {int(summary.iloc[0]['N_paper_sample']):,}",
                    f"- Finite color-feature rows: {int(summary.iloc[0]['N_finite_color_features']):,}",
                    f"- HST truth-labeled rows: {int(summary.iloc[0]['N_truth_labeled']):,}",
                    f"- Overall AUC where truth labels are available: {summary.iloc[0]['AUC_star']:.4f}",
                    "",
                    "Star-positive metrics at `pS_color > 0.5` are recorded in the CSV for each r-magnitude bin.",
                    "ECDFS/HST has a smaller truth-label sample than COSMOS/COSMOS2020.",
                ]
            )
            + "\n",
        )
    ...
    comparison_csv, comparison_md = _write_comparison(repo_root, summary)
    _write_report(
        repo_root,
        report,
        classifier_path,
    ...
    if comparison_csv is not None:
        summaries.append(comparison_csv)
    if comparison_md is not None:
        summaries.append(comparison_md)
    return {
